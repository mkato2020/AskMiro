...
@app.get("/api/admin/gate1-evidence")
def admin_gate1_evidence():
    """
    READ-ONLY diagnostic for Readiness v1 Gate 1 evidence collection.
    Runs exactly the five SELECT probes required by
    docs/READINESS_V1_GATE1_MIGRATION_DRY_RUN_REVIEW.md §7.1.

    No DDL. No DML. No env-var changes. No migration. Each probe runs in
    its own short transaction so one failure can't poison the rest.

    Intended to be removed once Gate 1 evidence is captured.
    """
    out = {"as_of": datetime.utcnow().isoformat() + "Z",
           "purpose": "readiness_v1_gate1_pre_migration_evidence"}

    def _q(sql, mode="all"):
        try:
            with db_pg.transaction() as conn:
                if mode == "one":
                    return db_pg.fetchone(conn, sql)
                return db_pg.fetchall(conn, sql)
        except Exception as e:
            return {"__err__": str(e)[:300]}

    # 1/5 — Postgres version
    out["pg_version"] = _q(
        "SELECT current_setting('server_version') AS pg_version, "
        "       current_setting('server_version_num')::int AS pg_num, "
        "       (current_setting('server_version_num')::int >= 110000) AS pg_ge_11",
        mode="one",
    )

    # 2/5 — Table existence for the 3 v1 tables
    out["v1_tables_present"] = _q(
        "SELECT t.target_table, "
        "       to_regclass('public.' || t.target_table)::text AS regclass_oid "
        "FROM (VALUES "
        "       ('enrichment_events'), "
        "       ('chain_operators'), "
    ...
    # 3/5 — Constraint existence
    out["constraint_collisions"] = _q(
        "SELECT conname, contype::text AS contype, convalidated, "
        "       pg_get_constraintdef(oid) AS definition "
        "FROM pg_constraint "
        "WHERE conname = 'entities_readiness_state_chk'",
    )

    # 4/5 — Index existence for ALL indexes 011 would create
    out["index_collisions"] = _q(
        "SELECT indexname, tablename, indexdef "
        "FROM pg_indexes "
        "WHERE schemaname = 'public' "
        "  AND indexname IN ( "
        "    'ix_entities_readiness_state', "
        "    'ix_entities_provenance', "
        "    'ix_enrichment_events_entity_time', "
        "    'ix_enrichment_events_outcome', "
        "    'ix_chain_operators_active_domain', "
        "    'ix_chain_operators_sector' "
        "  ) "
        "ORDER BY indexname",
    )

    # 5/5 — Column existence on entities
    out["entity_columns"] = _q(
        "SELECT column_name, data_type, is_nullable, column_default "
        "FROM information_schema.columns "
        "WHERE table_schema = 'public' "
        "  AND table_name = 'entities' "
        "  AND column_name IN ('readiness_state', "
        "                     'readiness_reason', "
        "                     'readiness_classified_at', "
        "                     'provenance') "
        "ORDER BY column_name",
    )

    # Bonus — schema_migrations table + 011 row
    out["schema_migrations_present"] = _q(
        "SELECT to_regclass('public.schema_migrations')::text AS oid",
        mode="one",
    )
    if isinstance(out["schema_migrations_present"], dict) \
            and out["schema_migrations_present"].get("oid"):
        out["migration_011_row"] = _q(
            "SELECT version, name, applied_at, applied_by "
            "FROM schema_migrations WHERE version = 11",
        )

    # Net interpretation, computed server-side for clarity
    tables_clear = all(
        (isinstance(r, dict) and r.get("regclass_oid") is None)
        for r in (out["v1_tables_present"] if isinstance(out["v1_tables_present"], list) else [])
    )
    constraint_clear = (
        isinstance(out["constraint_collisions"], list)
        and len(out["constraint_collisions"]) == 0
    )
    indexes_clear = (
        isinstance(out["index_collisions"], list)
        and len(out["index_collisions"]) == 0
    )
    columns_clear = (
        isinstance(out["entity_columns"], list)
        and len(out["entity_columns"]) == 0
    )
    pg_ok = (
        isinstance(out["pg_version"], dict)
        and out["pg_version"].get("pg_ge_11") is True
    )
    out["verdict"] = {
        "pg_version_ok":      pg_ok,
        "no_table_collision": tables_clear,
        "no_constraint":      constraint_clear,
        "no_index":           indexes_clear,
        "no_column":          columns_clear,
        "all_clear": (pg_ok and tables_clear and constraint_clear
                      and indexes_clear and columns_clear),
    }
    return out
